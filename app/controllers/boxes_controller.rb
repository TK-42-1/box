class BoxesController < ApplicationController
  ...
  def index
    @boxes = Box.filter(params[:filter]).deptfilter(params[:deptfilter]).search(params[:search]).order(sort_column + ' ' + sort_direction).paginate(page: params[:page], per_page: 25)
